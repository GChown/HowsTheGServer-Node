...
module.exports = function (app, connection, ws) {
  // Array of connected sockets - they get notified when update happens
  var socketList = [];

  // Votes websocket endpoint
  wsServer.on('request', function (request) {
      var connection = request.accept();
    //Add the connection to socket list
    socketList.push(connection);
    connection.on('close', function (reasonCode, description) {
        //Remove connection from socket list on disconnect
        socketList.splice(socketList.indexOf(connection), 1);
    });
  });

  // Average vote and number of votes
  ...
  app.get('/user/:username', function (req, res) {
    ...
    var returning = {};
    var select = 'SELECT COUNT (V_ID) votes FROM vote WHERE googleid = (SELECT googleid FROM user WHERE username=? collate utf8mb4_unicode_520_ci)';
